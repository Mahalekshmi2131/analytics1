...
abline(lm(df$Y ~ df$X), lty=3, lwd=4, col='green') # with regression line of best fit  to see how points are aorund lines of best fit.it reduces the errors in prediction
...
summary(fit1)
#if p value is less than 0.05 then things are significant,then the model exists.there is atlease one independent variable with high relation to y because of which we can predict y
#multiple r -sqaured =.9042 and it says 90 percent of variation in y can be determined through x
#Adjusted R-squared:- for multiple Lr
#Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
#there should be atleast single star in the above statement to accept the model.
...
#while [predicting we can only predict it within a range, not beyond that range]
#only interpolation is possible not extrapolation
...
# Predict Function for 4 values of X
...
par(mfrow=c(2,2))
plot(fit1)
#when residuals dont show a funnel shape then there is a linear relationship,residual should be random and normal distributed.
#Another assumption is 
par(mfrow=c(1,1))

#in the normal q-q graph the points should be along with the line
#in coooks distance
#learnhomoscedasiticity and heterosedacity for residual check whether it os correct
...
plot(fit1, which=4)
#13and11 seem to be outlier but they dont cross 0.5,it is within the range.
...
predict(fit,newdata=data.frame(X=mean(df$X)))# multiple R2 explains the variation,model fitness
