...
sales %>% group_by(partnum) %>% summarise(n = n()) %>% arrange(desc(n))
...
sales %>% mutate(newmargin=margin * 1.1) %>% select(partnum, margin, newmargin)

sales%>% filter(region=='01-East')%>% group_by(custname) %>% summarise(max(revenue))
sales%>% group_by(region,custname)%>% summarise(maxlev=max(revenue))%>%arrange(region,desc(custname))%>% top_n(n=2)